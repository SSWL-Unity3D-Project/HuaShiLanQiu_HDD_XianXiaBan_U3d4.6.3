...
public class LanQiuJianSu : SSGameMono
{
    /// <summary>
    /// 取消框.
    /// </summary>
    ...
    private void ClickTVYaoKongEnterBtEvent(InputEventCtrl.ButtonState val)
    {
        ...
        switch (m_XuanXiangState)
        {
            case SSGameDataCtrl.XuanXiangState.XuanXiang05:
                {
                    ...
                    //SSGameDataCtrl.GetInstance().m_SSUIRoot.SetActiveLanQiuJianSuDingBu(true);
                    //延迟显示顶部购买提示.
            ...
            case SSGameDataCtrl.XuanXiangState.XuanXiang04:
                {
                    SetIsLockPanel(true);
                    SSGameDataCtrl.GetInstance().m_SSUIRoot.SpawnGameLanQiuJianSu_ZhiFuPanel(m_PlayerIndex);
                    break;
                }
        }
    }
    ...
    public void RemoveSelf()
    {
        ...
        if (m_PlayerIndex != SSGameDataCtrl.PlayerIndex.Null)
        {
            if (SSGameDataCtrl.GetInstance().m_PlayerData[(int)m_PlayerIndex].IsJieShouTiaoZhan)
            {
                ...
                {
                    if (SSGameDataCtrl.GetInstance().m_BallSpawnArray[i].IndexCreatBallJieDuan == SSGameDataCtrl.GetInstance().IndexJieDuanLanQiuJianSu - 1)
                    {
                        //发球阶段在2时,增加发球阶段索引.
            ...
            {
                if (SSGameDataCtrl.GetInstance().m_BallSpawnArray[(int)m_PlayerIndex].IndexCreatBallJieDuan == SSGameDataCtrl.GetInstance().IndexJieDuanLanQiuJianSu - 1)
                {
                    //发球阶段在3时,增加发球阶段索引.
            ...
            //SSGameDataCtrl.GetInstance().TestResetPlayerBallSpeed(m_PlayerIndex); //test.
        }
        else
        {
            //玩家PK或者玩家主动触发道具购买.
        ...
        SSGameDataCtrl.GetInstance().m_SSUIRoot.SpawnGameLanQiuJianSuDingBuPanel();
